...
# Map-rotation player-count/St. Mary filter (and its three @hooks below)
# has been removed. The behaviour was brittle (race-y under fast joins,
# hard-coded map ids) and is superseded by the simpler `custom_tools.
# votemap_seeding` whitelist gate which is loaded for its side-effect
# of monkey-patching VoteMap.get_map_whitelist. See that module for the
# seeding-window definition (player count < 50 OR Kyiv hour ∈ [23, 6)).
import custom_tools.votemap_seeding as _votemap_seeding  # noqa: F401
