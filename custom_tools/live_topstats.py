...
# Translations
# "key" : ["english", "french", "german", "spanish", "polish", "brazilian-portuguese", "ukrainian"]
# Order matches common_translations.py: english, french, german, spanish, polish, brazilian portuguese, ukrainian (index 6)
# ----------------------------------------------

TRANSL = {
    "nostatsyet": ["No stats yet", "Pas de stats", "noch keine Statistiken", "Sin estadísticas aún", "Brak dostępnych statystyk", "Sem estatísticas ainda", "Ще немає статистики"],
    "allies": ["all", "all", "Allierte", "Aliados", "Alianci", "aliados", "Союзники"],
    "axis": ["axi", "axe", "Achsenmächte", "Eje", "Oś", "eixo", "Вісь"],
    "best_players": ["Best players", "Meilleurs joueurs", "Beste Spieler", "Mejores jugadores", "Najlepsi gracze", "Melhores jogadores", "Найкращі гравці"],
    "armycommander": ["Commander", "Commandant", "Kommandant", "Comandante", "Dowódca", "Comandante", "Командир"],
    "infantry": ["Infantry", "Infanterie", "Infanterie", "Infantería", "Piechota", "Infantaria", "Піхота"],
    "tankers": ["Tankers", "Tankistes", "Panzerspieler", "Tanquistas", "Czołgiści", "Tanqueiros", "Танкісти"],
    "best_squads": ["Best squads", "Meilleures squads", "Beste Mannschaften", "Mejores escuadras", "Najlepsze jednostki", "Melhores esquadrões", "Найкращі відділи"],
    "offense": ["attack", "attaque", "Angriff", "Ataque", "Ofensywa", "ataque", "Напад"],
    "defense": ["defense", "défense", "Verteidigung", "Defensa", "Defensywa", "defesa", "Захист"],
    "combat": ["combat", "combat", "Kampf", "Combate", "Walka", "combate", "Бойова ефективність"],
    "support": ["support", "soutien", "Unterstützung", "Apoyo", "Wsparcie", "suporte", "Підтримка"],
    "ratio": ["ratio", "ratio", "Verhältnis", "Ratio", "Średnia", "proporção", "Співвідношення"],
    "killrate": ["kills/min", "kills/min", "Kills/min", "bajas/min", "Zabójstwa/min", "abates/min", "вбивства/хв"],
    "vip_until": ["VIP until", "VIP jusqu'au", "VIP bis", "VIP hasta", "VIP do", "VIP até", "VIP до"],
    "already_vip": ["Already VIP !", "Déjà VIP !", "bereits VIP !", "¡Ya es VIP!", "Aktualnie ma VIPa!", "Já é VIP!", "Вже VIP !"],
    "gamejustended": ["Game just ended", "Partie terminée", "Spiel beendet", "La partida terminó", "Gra właśnie się zakończyła", "Jogo acabou", "Гра щойно закінчилася"],
    "vip_at": ["at", "à", "um", "a las", "do godziny", "às", "о"]
}
...
def stats_display(
    ...
    """
    Format the message sent (bilingual: English + Ukrainian)
    """
    if OFFENSEDEFENSE_RATIO == 0:
    ...
    # Build message in English (LANG=0)
    LANG_EN = 0
    message_en = ""
    # players
    if (
        ...
        message_en = f"█ {TRANSL['best_players'][LANG_EN]} █\n\n"
        # players / commanders
        if len(top_commanders_teamplay) != 0:
            message_en += (
                f"▓ {TRANSL['armycommander'][LANG_EN]} ▓\n\n"
                f"─ {TRANSL['combat'][LANG_EN]} + ({TRANSL['support'][LANG_EN]} * {str(combatsupport_ratio)}) ─\n{top_commanders_teamplay}\n"
            )
        # players / infantry
        if (
            ...
            message_en += f"▓ {TRANSL['infantry'][LANG_EN]} ▓\n\n"
            if len(top_infantry_offdef) != 0:
                message_en += f"─ {TRANSL['offense'][LANG_EN]} + ({TRANSL['defense'][LANG_EN]} * {str(offensedefense_ratio)}) ─\n{top_infantry_offdef}\n"
            if len(top_infantry_teamplay) != 0:
                message_en += f"─ {TRANSL['combat'][LANG_EN]} + ({TRANSL['support'][LANG_EN]} * {str(combatsupport_ratio)}) ─\n{top_infantry_teamplay}\n"
            if len(top_infantry_ratio) != 0:
                message_en += f"─ {TRANSL['ratio'][LANG_EN]} ─\n{top_infantry_ratio}\n"
            if len(top_infantry_killrate) != 0:
                message_en += f"─ {TRANSL['killrate'][LANG_EN]} ─\n{top_infantry_killrate}\n"
    # squads
    if (
    ...
    if (
        ...
        message_uk = f"█ {TRANSL['best_players'][LANG_UK]} █\n\n"
        # players / commanders
        if len(top_commanders_teamplay) != 0:
            message_uk += (
                f"▓ {TRANSL['armycommander'][LANG_UK]} ▓\n\n"
                f"─ {TRANSL['combat'][LANG_UK]} + ({TRANSL['support'][LANG_UK]} * {str(combatsupport_ratio)}) ─\n{top_commanders_teamplay}\n"
            )
        # players / infantry
        if (
            len(top_infantry_offdef) != 0
            ...
            message_uk += f"▓ {TRANSL['infantry'][LANG_UK]} ▓\n\n"
            if len(top_infantry_offdef) != 0:
                message_uk += f"─ {TRANSL['offense'][LANG_UK]} + ({TRANSL['defense'][LANG_UK]} * {str(offensedefense_ratio)}) ─\n{top_infantry_offdef}\n"
            if len(top_infantry_teamplay) != 0:
                message_uk += f"─ {TRANSL['combat'][LANG_UK]} + ({TRANSL['support'][LANG_UK]} * {str(combatsupport_ratio)}) ─\n{top_infantry_teamplay}\n"
            if len(top_infantry_ratio) != 0:
                message_uk += f"─ {TRANSL['ratio'][LANG_UK]} ─\n{top_infantry_ratio}\n"
            if len(top_infantry_killrate) != 0:
                message_uk += f"─ {TRANSL['killrate'][LANG_UK]} ─\n{top_infantry_killrate}\n"
    # squads
    if (
        len(top_squads_infantry_offdef) != 0
        or len(top_squads_infantry_teamplay) != 0
        ...
        # squads / armor
        if len(top_squads_armor_offdef) != 0 or len(top_squads_armor_teamplay) != 0:
            message_uk += f"▓ {TRANSL['tankers'][LANG_UK]} ▓\n\n"
            if len(top_squads_armor_offdef) != 0:
                message_uk += f"─ {TRANSL['offense'][LANG_UK]} + ({TRANSL['defense'][LANG_UK]} * {str(offensedefense_ratio)}) ─\n{top_squads_armor_offdef}\n"
            if len(top_squads_armor_teamplay) != 0:
                message_uk += f"─ {TRANSL['combat'][LANG_UK]} + ({TRANSL['support'][LANG_UK]} * {str(combatsupport_ratio)}) ─\n{top_squads_armor_teamplay}\n"
    ...
    # If no data yet
    if len(message_en) == 0:
        return f"{TRANSL['nostatsyet'][LANG_EN]} / {TRANSL['nostatsyet'][LANG_UK]}"
...
def stats_on_match_end(
    ...
    if server_number in ENABLE_ON_SERVERS:
        ...
        # Для match_end нет конкретного player_id, показываем билингвально
        message = stats_display(
            top_commanders_teamplay,
            top_infantry_offdef,
            top_infantry_teamplay,
            ...
            top_squads_armor_offdef,
            top_squads_armor_teamplay,
            player_id=None  # Билингвальный режим для всех
        )
